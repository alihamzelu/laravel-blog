...
<head>
  ...
  <title>Author Dashboard - {{ $user->name }}</title>
  ...
  <main class="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
    ...
    <header class="rounded-2xl border border-slate-200 bg-white p-6 md:p-8 shadow-sm dark:border-slate-800 dark:bg-slate-900/40 mb-12 transition-colors">
      <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-6">
        ...
        <div class="flex items-center space-x-5">
          <img class="h-16 w-16 sm:h-20 sm:w-20 rounded-full object-cover ring-4 ring-slate-100 dark:ring-slate-800" src="{{ asset('storage/'. $user->profile->avatar) }}" alt="avatar">
          <div>
            <h1 class="text-xl sm:text-2xl font-black tracking-tight text-slate-900 dark:text-white">{{ $user->username }}</h1>
            <p class="text-xs sm:text-sm text-slate-400 dark:text-slate-500 font-medium">{{ $user->job ?? 'Member' }}</p>
        ...
        <div class="flex-shrink-0 sm:self-start">
          @if(auth()->check() && (auth()->id() == $user->id || auth()->user()->hasRole('admin')) && $user->hasAnyRole(['admin', 'author']))
          <a href="{{ route('create') }}"
            class="w-full sm:w-auto inline-flex items-center justify-center space-x-2 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-3 text-sm font-bold shadow-md hover:shadow-lg transition-all dark:bg-emerald-400 dark:text-night dark:hover:bg-emerald-500">
    ...
    <section>
      <div class="border-b border-slate-200 dark:border-slate-800 pb-4 mb-6 flex items-center justify-between transition-colors">
        <h2 class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
          ...
          @else
          {{ $user->name }}'s Published Content (<span id="post-counter">{{ $postCount }}</span>)
          @endif
        </h2>
      </div>
      ...
      <div id="author-posts-grid" class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        ...
        <div class="col-span-full flex flex-col items-center justify-center text-center p-12 bg-white border border-slate-200 dark:bg-slate-900/40 dark:border-slate-800 rounded-2xl shadow-sm transition-all">
          ...
          <p class="mt-1 text-sm text-slate-500 dark:text-slate-400 max-w-xs">It looks like there aren't any posts published yet.</p>
          @if(auth()->check() && auth()->id() == $user->id)
    ...
    <section class="mt-16">
      <div class="border-b border-slate-200 dark:border-slate-800 pb-4 mb-6 flex items-center justify-between transition-colors">
        <h2 class="text-xs font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500">
          @if(auth()->check() && auth()->id() == $user->id)
          Your Visual Galleries (<span>{{ count($galleries) }}</span>)
          @else
          {{ $user->name }}'s Visual Galleries (<span>{{ count($galleries) }}</span>)
          @endif
        </h2>
      </div>
      ...
      <div class="grid gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        <div class="group flex flex-col overflow-hidden rounded-2xl bg-white border border-slate-200/80 transition-all duration-200 hover:-translate-y-1 hover:shadow-lg hover:border-indigo-500 dark:bg-slate-900/40 dark:border-slate-800 dark:hover:border-emerald-500 dark:hover:shadow-emerald-500/5">
          ...
          <div class="flex items-center justify-end space-x-1 border-t border-slate-100 px-4 py-2.5 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-800/20">
            @if(auth()->check() && (auth()->id() == $gallery->user_id || auth()->user()->hasRole('admin')))
    ...
    @else
    <div class="text-center py-16 bg-white dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 rounded-2xl">
      <p class="text-slate-500 dark:text-slate-400">This user is a Reader and hasn't published any content yet.</p>
    </div>
    @endif
